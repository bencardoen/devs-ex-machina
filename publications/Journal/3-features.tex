...
We argue that a general purpose simulation tool should support single core, conservative, and optimistic synchronization, as is the case for \textit{dxex}. Futhermore it should be possible to combine each inter core synchronization protocol with the parallel execution of events defined by \pSim.
...
% protocol or inherent parallelism?
\subsubsection{\pSim}
...
\textit{Dxex} uses \textit{Boost Pool}~\cite{boostpool} allocators in multi core simulation kernels and arena-style allocators for single core simulation.
