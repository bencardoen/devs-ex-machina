...
With respect to the theoretical analysis in~\cite{amdahlpdevs} this gives a parameter value $q = 1$. Q is the probability that a receiving model becomes immediately imminent (i.e. scheduled in the next timestep). 
Without random scheduling the receiving model becomes immediately imminent. With random scheduling the receiving model becomes imminent with a scheduled time in [1, timestep]. This would result in a probability $q = \frac{n}{timestep}$ with n the receiving models at that point in time. The benchmarks are run sufficiently long enough to guarantee that the frequency of internal and external transitions is equal between random and non-random benchmarks. This results in n equal to all models in the benchmark. Only the concurrency of events differs. 
