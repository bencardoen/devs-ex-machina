...
With respect to the theoretical analysis ~\cite{amdahlpdevs} this gives a parameter value $q = 1$ for a fixed timeadvance and $q=0.5$ for a random timeadvance. Q is the probability that a receiving model becomes immediately imminent. 
Without a fixed timeadvance the receiving model becomes immediately imminent. 
With random scheduling the receiving model becomes imminent with a scheduled time in the current timeslot or the next, chosen with a uniform distribution, therefore $q = \frac{1}{2}$.
 The benchmarks are run sufficiently long enough to guarantee that the frequency of internal and external transitions is equal between random and non-random benchmarks. The key difference is that although the event frequency is the same, their concurrency is not.
