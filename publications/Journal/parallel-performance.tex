...
\begin{figure}
	...
	\includegraphics[width=\modelfraction\columnwidth, height=8cm, keepaspectratio]{fig/queue_allocation.eps}
...
The difference between depth first and breadth first allocated kernels is striking, the first results in sublinear speedup for both synchronization variants. %reword 
...
In dxex state saving is never required for a conservative simulation, this is handled transparently for the user who need not implement this behavior. 
...
% Conservative
Conservative has a more nuanced speedup behavior in this benchmark. We have detailed how a conservative kernel in dxex scales linear in the number of atomic models (if lookahead is $\epsilon$), this effect is more clearly visible here. \\
...
The simulation kernels have a non trivial overhead compared to a sequential kernel. In this section we will try to isolate this overhead and highlight the consequences this has for applying a synchronization protocol in real life models.
Profiling indicates that there are in the most trivial of models only 2 causes for computational load outside of the actual simulation : memory management and random number generation. 
...
A sequential simulation kernel can leverage the invariant that no allocated event will persist beyond a single time advance and use a arena-style allocator. Conservative and optimistic simulation need to use generic pool allocators. \\
