...
Synchronization is required for PDEVS to prevent or recover from causality violations if the simulation is executed in parallel. Preventing causality violations (conservative) typically requires domain-specific information to compensate for performance loss, while recovering from causality errors (optimistic) requires a state to recover to.
