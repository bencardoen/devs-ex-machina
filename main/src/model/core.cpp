...
n_model::Core::Core():m_network(nullptr), m_time(0,0), m_coreid(0),m_live(false){
	m_scheduler = n_tools::SchedulerFactory<ModelEntry>::makeScheduler(n_tools::Storage::BINOMIAL, false);
}

n_model::Core::Core(std::size_t id, const t_networkptr& netlink):m_network(netlink), m_time(0,0), m_coreid(id), m_live(false){
	m_scheduler = n_tools::SchedulerFactory<ModelEntry>::makeScheduler(n_tools::Storage::BINOMIAL, false);
}
...
void
n_model::Core::receiveMessages(std::vector<t_msgptr>&){
