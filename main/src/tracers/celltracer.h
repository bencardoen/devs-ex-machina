...
private:
	...
	inline void traceCall(const n_model::t_atomicmodelptr& adevs, std::size_t coreid, t_timestamp time){
		...
		std::function<void()> fun = std::bind(&t_derived::doTrace, static_cast<Derived*>(this), time, celldevs->getPoint(), celldevs->getState()->toCell());
	...
	inline std::enable_if<std::is_same<OutputPolicy, MultiFileWriter>::value>::type
